...
void assert_empty_arg_list()
{
	...
	helper_function_list_emplace_back(list, 0, "test", nullptr,
					  EBPF_RETURN_TYPE_INTEGER, 0, nullptr);

	marshall_bpf_helper_definitions(list, append_result);
	assert("test()->" + std::to_string(EBPF_RETURN_TYPE_INTEGER) == buffer);
...
void assert_many_args()
{
	...
	ebpf_argument_type_t arg_types[] = {
	    EBPF_ARGUMENT_TYPE_PTR_TO_READABLE_MEM_OR_NULL,
	    EBPF_ARGUMENT_TYPE_CONST_SIZE,
	    EBPF_ARGUMENT_TYPE_CONST_SIZE_OR_ZERO};
	helper_function_list_emplace_back(list, 0, "test", nullptr,
					  EBPF_RETURN_TYPE_UNSUPPORTED, 3,
					  arg_types);

	marshall_bpf_helper_definitions(list, append_result);
	std::stringstream stream;
	stream << std::hex << EBPF_ARGUMENT_TYPE_PTR_TO_READABLE_MEM_OR_NULL;

	assert("test(" + stream.str() + ","
		   + std::to_string(EBPF_ARGUMENT_TYPE_CONST_SIZE) + ","
		   + std::to_string(EBPF_ARGUMENT_TYPE_CONST_SIZE_OR_ZERO)
		   + ")->" + std::to_string(EBPF_RETURN_TYPE_UNSUPPORTED)
	       == buffer);
...
void assert_many_functions()
{
	...
	ebpf_argument_type_t arg_types[] = {
	    EBPF_ARGUMENT_TYPE_ANYTHING, EBPF_ARGUMENT_TYPE_CONST_SIZE,
	    EBPF_ARGUMENT_TYPE_CONST_SIZE_OR_ZERO};
	...
	marshall_bpf_helper_definitions(list, append_result);
	assert("test()->" + std::to_string(EBPF_RETURN_TYPE_INTEGER) + ";"
		   + "test2(" + std::to_string(EBPF_ARGUMENT_TYPE_ANYTHING)
		   + "," + std::to_string(EBPF_ARGUMENT_TYPE_CONST_SIZE) + ","
		   + std::to_string(EBPF_ARGUMENT_TYPE_CONST_SIZE_OR_ZERO)
		   + ")->" + std::to_string(EBPF_RETURN_TYPE_UNSUPPORTED)
	       == buffer);
...
void assert_unmarshal_empty()
{
	...
	assert(result2->m_head->m_function_signature.m_return_type == (ebpf_return_type_t)10);
}
